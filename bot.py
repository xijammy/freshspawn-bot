...
# Fresh Spawn role (by name; if you know the ID, tell me and I’ll swap to ID for extra reliability)
...
async def resolve_member_from_dyno_join_log(message: discord.Message) -> discord.Member | None:
    """
    Dyno 'Member Joined' embed usually mentions the user.
    Prefer message.mentions; fallback to parsing embed description for <@id>.
    """
    if message.mentions:
...
async def resolve_ticket_owner_from_channel(channel: discord.TextChannel) -> discord.Member | None:
    """
    Your Ticket Tool posts an opening message that mentions the ticket owner (per your screenshot).
...
async def on_message(message: discord.Message):
    ...
    # ---- Dyno join logs -> start timer + public welcome ----
    if message.channel.id == DYNO_LOG_CHANNEL_ID and message.author.id == DYNO_BOT_ID:
        if not message.embeds:
            return

        emb = message.embeds[0]
        if (emb.title or "").strip().lower() != "member joined":
            return

        member = await resolve_member_from_dyno_join_log(message)
        if member is None:
            print("[WARN] Dyno join log detected but couldn't resolve member.")
            return
