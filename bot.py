...
async def resolve_member_from_dyno_join_log(message: discord.Message) -> discord.Member | None:
    """
    Dyno join card (your screenshot) contains the numeric user id in a field like:
      "ID: 1477... • Today at 15:00"
    This works even when there is NO <@mention> in the embed.
    """
    ...
    # Build a big text blob from all embed locations
    parts: list[str] = []
    if emb.title:
    ...
    # 1) If a real mention exists, use it
    m = re.search(r"<@!?(\d{17,20})>", text)
    if m:
        user_id = int(m.group(1))
    else:
        # 2) Prefer Dyno's ID: format
        m2 = re.search(r"\bID:\s*(\d{17,20})\b", text)
        if not m2:
            print(f"[DEBUG] Dyno join embed text (no ID found): {text!r}")
            return None
...
async def on_message(message: discord.Message):
    ...
    # Dyno sometimes uses title, sometimes author name
    title = (emb.title or "").strip().lower()
    author_name = ((emb.author.name if emb.author else "") or "").strip().lower()

    print(f"[DEBUG] Dyno message received: title='{emb.title}' author='{emb.author.name if emb.author else None}' fields={len(emb.fields)}")

    if ("member joined" not in title) and ("member joined" not in author_name):
        return

    member = await resolve_member_from_dyno_join_log(message)
    if member is None:
        print("[WARN] Dyno join log detected but couldn't resolve member (no ID/mention found).")
        return
